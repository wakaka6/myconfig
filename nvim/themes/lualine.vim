...
function custom_fname:init(options)
  ...
  self.status_colors = {
    saved = highlight.create_component_highlight_group(
      {fg = default_status_colors.saved}, 'filename_status_saved', self.options), modified = highlight.create_component_highlight_group( {fg = default_status_colors.modified}, 'filename_status_modified', self.options), }
...
line.setup {
  ...
  sections = {
    ...
    lualine_z = {
        { function () return '%3p%%' end, right_padding = 0 },
        { function () return [[%l %c]] end , separator = { right = '' }, left_padding = 0 }
        },
  },
  inactive_sections = {
    lualine_a = {'filetype'},
